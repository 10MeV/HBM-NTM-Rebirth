...
- 多方块：
  ...
  - 保留旧 `getOffset() = 1` 的放置语义：点击点沿 facing 前方一格成为 core。
  - 旧 XR 尺寸 `{U,D,N,S,W,E} = {2,0,1,1,1,1}` 在现代 helper 中换算为 `{+X,-X,+Y,-Y,+Z,-Z} = {1,1,2,0,1,1}`，用于 dummy 占位和拆除清理。
- 渲染：
  - 复制旧 OBJ 与贴图到现代资源树。
  ...
  - dummy 方块保持碰撞/交互/破坏联动，但 `RenderShape.INVISIBLE`，不参与多方块视觉显示。
