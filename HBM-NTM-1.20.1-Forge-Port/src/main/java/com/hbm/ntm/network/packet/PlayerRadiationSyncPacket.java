...
public record PlayerRadiationSyncPacket(float radiation, float environment, float chunkRadiation, float resistance) {
    public static PlayerRadiationSyncPacket decode(FriendlyByteBuf buffer) {
        return new PlayerRadiationSyncPacket(buffer.readFloat(), buffer.readFloat(), buffer.readFloat(), buffer.readFloat());
    }
    ...
    public static void handle(PlayerRadiationSyncPacket packet, Supplier<NetworkEvent.Context> contextSupplier) {
        ...
        context.enqueueWork(() -> DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () ->
                ClientRadiationData.update(packet.radiation, packet.environment, packet.chunkRadiation, packet.resistance)));
