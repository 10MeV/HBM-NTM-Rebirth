...
/**
 * Legacy 1.7.10 generic machine recipe object facade.
 *
 * <p>The modern runtime remains datapack-backed. This class preserves the old
 * mutable recipe DTO/API shape for source migration and legacy JSON tooling.</p>
 */
...
public class GenericRecipe {
    ...
    public GenericRecipe setIcon(ItemLike item) {
        return setIcon(new ItemStack(item));
    }

    public GenericRecipe setIcon(Block block) {
        return setIcon((ItemLike) block);
    }
    ...
    public GenericRecipe setPools(String... pools) {
        ...
        for (String pool : pools) {
            if (!GeneralConfig.enable528 && pool.startsWith(GenericRecipes.POOL_PREFIX_528)) {
    ...
    /**
     * Only for recipe config/import paths. Mirrors the old method by skipping
     * the default 528 guard, but still only mutates this in-memory facade.
     */
    ...
    public GenericRecipe inputItems(AStack... input) {
        ...
        for (AStack stack : this.inputItem) {
            checkStackLimit(stack);
        }
        return this;
    }
    ...
    public GenericRecipe outputItems(IOutput... output) {
    ...
    public GenericRecipe outputItems(ItemStack... output) {
        ...
        for (int i = 0; i < outputItem.length; i++) {
            outputItem[i] = new ChanceOutput(output[i]);
        }
        return this;
    }
    ...
    private void checkStackLimit(AStack stack) {
        ...
        if (stack instanceof ComparableStack comparable) {
            ItemStack itemStack = comparable.toStack();
            max = comparable.item.getMaxStackSize(itemStack);
        }
        if (stack.stacksize > max) {
            throw new IllegalArgumentException("AStack " + stack + " in " + this.name + " exceeds stack limit of " + max + "!");
        }
    }
    ...
    public ItemStack getIcon() {
        if (icon == null) {
            if (outputItem != null) {
                if (outputItem[0] instanceof ChanceOutput chance) {
                    icon = chance.stack.copy();
                } else if (outputItem[0] instanceof ChanceOutputMulti multi) {
                    icon = multi.pool.get(0).stack.copy();
                }
                return icon;
            }
            if (outputFluid != null) {
        ...
        if (icon == null) {
            icon = new ItemStack(ModItems.NOTHING.get());
        }
        return icon;
    }
    ...
    public String getLocalizedName() {
        ...
        if (customLocalization) {
            resolved = I18nUtil.resolveKey(name);
        }
        if (resolved == null) {
            resolved = getIcon().getHoverName().getString();
        }
        if (nameWrapper != null) {
            resolved = I18nUtil.resolveKey(nameWrapper, resolved);
        }
        return resolved;
    }

    public void printNEIExtras() {
        // Old NEI drew directly here. Modern JEI/screens read getDisplayLines().
    }
    ...
    protected void autoSwitch(List<String> list) {
        if (autoSwitchGroup != null) {
            String[] lines = I18nUtil.resolveKeyArray("autoswitch", I18nUtil.resolveKey(autoSwitchGroup));
    ...
    protected void input(List<String> list) {
        ...
        if (inputFluid != null) {
            for (FluidStack fluid : inputFluid) {
                list.add("  " + ChatFormatting.BLUE + fluid.fill + "mB " + fluid.type.getLocalizedName()
                        + (fluid.pressure == 0 ? "" : " " + I18nUtil.resolveKey("gui.recipe.atPressure") + " "
                        + ChatFormatting.RED + fluid.pressure + " PU"));
            }
        }
    }
    ...
    protected void output(List<String> list) {
        ...
        if (outputFluid != null) {
            for (FluidStack fluid : outputFluid) {
                String pressurePart = fluid.pressure == 0 ? "" : " " + I18nUtil.resolveKey("gui.recipe.atPressure")
                        + " " + ChatFormatting.RED + fluid.pressure + " PU";
