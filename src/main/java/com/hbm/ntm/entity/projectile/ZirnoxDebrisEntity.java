...
import com.hbm.ntm.neutron.RBMKDebrisPlanner.ZirnoxDebrisPlan;
...
public class ZirnoxDebrisEntity extends Entity {
    ...
    public ZirnoxDebrisEntity(Level level) {
        this(ModEntityTypes.ZIRNOX_DEBRIS.get(), level);
    }

    public ZirnoxDebrisEntity(Level level, double x, double y, double z, ZirnoxDebrisType type) {
        this(level);
        setPos(x, y, z);
        setDebrisType(type);
    }
    ...
    private void applyRadiationAura(ZirnoxDebrisPlan plan) {
    ...
    public void setDebrisType(ZirnoxDebrisType type) {
        ZirnoxDebrisType safeType = type == null ? ZirnoxDebrisType.BLANK : type;
        entityData.set(DATA_TYPE, safeType.ordinal());
        refreshDimensions();
    }

    public ZirnoxDebrisType getDebrisType() {
        return RBMKDebrisPlanner.zirnoxTypeFromLegacyOrdinal(entityData.get(DATA_TYPE)).type();
    }

    private ZirnoxDebrisPlan plan() {
        return RBMKDebrisPlanner.zirnox(getDebrisType());
    }
