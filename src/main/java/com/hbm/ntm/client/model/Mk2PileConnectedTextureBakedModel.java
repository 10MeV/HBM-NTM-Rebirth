...
import com.hbm.ntm.blockentity.Mk2PileConnectedTextureData;
...
/** Rebuilds the four-fragment-per-face rendering of 1.7.10 {@code RenderBlocksCT}. */
public final class Mk2PileConnectedTextureBakedModel implements BakedModel {
    private static final ResourceLocation MODEL_ID = new ResourceLocation(HbmNtm.MOD_ID, "block/pile_block_connected");
    private static final FaceBakery FACE_BAKERY = new FaceBakery();

    private final TextureAtlasSprite side;
    private final TextureAtlasSprite top;
    private final TextureAtlasSprite sideCt;
    private final TextureAtlasSprite topCt;
    private final TextureAtlasSprite inputCt;
    private final TextureAtlasSprite outputCt;
    private final TextureAtlasSprite controlTopCt;
    private final TextureAtlasSprite coreCt;
    private final ItemTransforms transforms;
    private final Map<Long, List<BakedQuad>> quadCache = new ConcurrentHashMap<>();

    public Mk2PileConnectedTextureBakedModel(TextureAtlasSprite side, TextureAtlasSprite top, TextureAtlasSprite sideCt,
            TextureAtlasSprite topCt, TextureAtlasSprite inputCt, TextureAtlasSprite outputCt,
            TextureAtlasSprite controlTopCt, TextureAtlasSprite coreCt, ItemTransforms transforms) {
        this.side = side;
        this.top = top;
        this.sideCt = sideCt;
        this.topCt = topCt;
        this.inputCt = inputCt;
        this.outputCt = outputCt;
        this.controlTopCt = controlTopCt;
        this.coreCt = coreCt;
    ...
    public @NotNull List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side,
            @NotNull RandomSource random, @NotNull ModelData data, @Nullable net.minecraft.client.renderer.RenderType renderType) {
        if (side != null || renderType != null && renderType != net.minecraft.client.renderer.RenderType.solid()) return List.of();
        Mk2PileStructureBlock.Role role = state != null && state.hasProperty(Mk2PileStructureBlock.ROLE)
                ? state.getValue(Mk2PileStructureBlock.ROLE) : Mk2PileStructureBlock.Role.DUMMY;
        Long connections = data.get(Mk2PileConnectedTextureData.CONNECTION_MASK);
        long key = ((connections == null ? 0L : connections) << 4) | role.ordinal();
        return quadCache.computeIfAbsent(key, ignored -> buildQuads(role, connections == null ? 0L : connections));
    }

    private List<BakedQuad> buildQuads(Mk2PileStructureBlock.Role role, long connections) {
        List<BakedQuad> quads = new ArrayList<>(24);
        for (Direction face : Direction.values()) {
            boolean horizontal = face.getAxis().isHorizontal();
            TextureAtlasSprite full = horizontal ? side : top;
            TextureAtlasSprite connected = horizontal ? sideTexture(role) : topTexture(role);
    ...
    private TextureAtlasSprite sideTexture(Mk2PileStructureBlock.Role role) {
        return switch (role) {
            case FUEL_IN, AIR_IN -> inputCt;
            case FUEL_OUT, AIR_OUT -> outputCt;
            case CORE -> coreCt;
            default -> sideCt;
        };
    }

    private TextureAtlasSprite topTexture(Mk2PileStructureBlock.Role role) {
        return role == Mk2PileStructureBlock.Role.CONTROL ? controlTopCt : topCt;
    }
    ...
    private void addFragment(List<BakedQuad> quads, Direction face, int fragment, int type, TextureAtlasSprite full,
        ...
        float[] uv = uv(type);
        BlockElementFace elementFace = new BlockElementFace(null, -1, "#ct", new BlockFaceUV(uv, 0));
    ...
    private static Vector3f[] bounds(Direction face, int fragment) {
        ...
        return switch (face) {
            ...
            case EAST -> pair(low, b0, right ? 0.0F : 8.0F, high, b1, right ? 8.0F : 16.0F);
        };
    }
    ...
    @Override @Deprecated public TextureAtlasSprite getParticleIcon() { return side; }
    @Override public TextureAtlasSprite getParticleIcon(@NotNull ModelData data) { return side; }
