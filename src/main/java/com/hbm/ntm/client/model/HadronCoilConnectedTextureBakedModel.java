...
import com.hbm.ntm.blockentity.HadronCoilConnectedTextureData;
...
public final class HadronCoilConnectedTextureBakedModel implements BakedModel {
    private static final ResourceLocation MODEL_ID = new ResourceLocation(HbmNtm.MOD_ID,
            "block/hadron_coil_connected");
    ...
    public HadronCoilConnectedTextureBakedModel(TextureAtlasSprite block, TextureAtlasSprite blockCt,
    ...
    public @NotNull List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side,
        ...
        if (side != null || renderType != null && renderType != net.minecraft.client.renderer.RenderType.solid()) {
            return List.of();
        }
        Long connections = data.get(HadronCoilConnectedTextureData.CONNECTION_MASK);
        long key = connections == null ? 0L : connections;
        return quadCache.computeIfAbsent(key, ignored -> buildQuads(key));
    }

    private List<BakedQuad> buildQuads(long connections) {
        List<BakedQuad> quads = new ArrayList<>(24);
        for (Direction face : Direction.values()) {
            int neighborhood = (int) (connections >>> (face.ordinal() * 8)) & 0xFF;
            addFace(quads, face, neighborhood);
        }
        return List.copyOf(quads);
    }

    private void addFace(List<BakedQuad> quads, Direction face, int neighbors) {
        addFragment(quads, face, 0, fragmentType(bit(neighbors, 3), bit(neighbors, 0), bit(neighbors, 1), false, false));
        addFragment(quads, face, 1, fragmentType(bit(neighbors, 4), bit(neighbors, 2), bit(neighbors, 1), true, false));
        addFragment(quads, face, 2, fragmentType(bit(neighbors, 3), bit(neighbors, 5), bit(neighbors, 6), false, true));
        addFragment(quads, face, 3, fragmentType(bit(neighbors, 4), bit(neighbors, 7), bit(neighbors, 6), true, true));
    }
    ...
    private static int fragmentType(boolean horizontal, boolean corner, boolean vertical, boolean right, boolean bottom) {
        int type = vertical && horizontal && corner ? 4 : vertical && horizontal ? 8 : vertical ? 16
                : horizontal ? 12 : 0;
        return type | (right ? 1 : 0) | (bottom ? 2 : 0);
    }

    private void addFragment(List<BakedQuad> quads, Direction face, int fragment, int type) {
        Vector3f[] bounds = bounds(face, fragment);
        TextureAtlasSprite sprite = type < 4 ? block : blockCt;
