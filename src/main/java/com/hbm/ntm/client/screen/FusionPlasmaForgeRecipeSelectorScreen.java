...
public class FusionPlasmaForgeRecipeSelectorScreen extends Screen {
    ...
    private final FusionPlasmaForgeScreen previousScreen;
    ...
    protected FusionPlasmaForgeRecipeSelectorScreen(FusionPlasmaForgeScreen previousScreen) {
        super(Component.translatableWithFallback("container.machinePlasmaForge", "Plasma Forge"));
        this.previousScreen = previousScreen;
        FusionPlasmaForgeBlockEntity blockEntity = previousScreen.getMenu().getBlockEntity();
        this.selection = blockEntity.getSelectedRecipeName();
        this.allRecipes = GenericMachineRecipeSelector.recipes(blockEntity.getLevel(),
                GenericMachineRecipe.Machine.PLASMA_FORGE,
                blockEntity.getItems().getStackInSlot(FusionPlasmaForgeBlockEntity.SLOT_BLUEPRINT));
        regenerateRecipes("");
    }
    ...
    private void renderHighlights(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        if (isInside(mouseX, mouseY, 152, 18, 16, 16)) graphics.blit(TEXTURE, leftPos + 152, topPos + 18, 176, 0, 16, 16);
        if (isInside(mouseX, mouseY, 152, 36, 16, 16)) graphics.blit(TEXTURE, leftPos + 152, topPos + 36, 176, 16, 16, 16);
        if (isInside(mouseX, mouseY, 152, 90, 16, 16)) graphics.blit(TEXTURE, leftPos + 152, topPos + 90, 176, 32, 16, 16);
        if (isInside(mouseX, mouseY, 134, 108, 16, 16)) graphics.blit(TEXTURE, leftPos + 134, topPos + 108, 176, 48, 16, 16);
    }
    ...
    private void sendSelection() {
        ...
        ModMessages.sendTileControl(previousScreen.getMenu().getBlockEntity().getBlockPos(),
                FusionPlasmaForgeBlockEntity.recipeSelectionTag(selection));
    }
}
