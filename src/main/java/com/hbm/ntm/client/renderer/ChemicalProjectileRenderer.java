...
public class ChemicalProjectileRenderer extends EntityRenderer<ChemicalProjectileEntity> {
    ...
    private void renderGas(ChemicalProjectileEntity entity, ChemicalStyle style, float partialTick,
        ...
        LegacyTexturedQuadRenderer.quadDirect(PARTICLE, poseStack, buffer, LightTexture.FULL_BRIGHT,
                OverlayTexture.NO_OVERLAY, LegacyTexturedRenderMode.TRANSLUCENT, 0.0F, 0.0F, 1.0F,
                -size, -size, 0.0F, 1 - uMirror, 1 - vMirror,
                size, -size, 0.0F, uMirror, 1 - vMirror,
                size, size, 0.0F, uMirror, vMirror,
                -size, size, 0.0F, 1 - uMirror, vMirror,
                color, alpha);
        poseStack.popPose();
    }
    ...
    private static void renderBeam(ChemicalProjectileEntity entity, float partialTick, PoseStack poseStack,
        ...
        beamFace(consumer, pose, -size, -size, size, -size, length, nearAlpha);
        beamFace(consumer, pose, -size, size, size, size, length, nearAlpha);
        beamFace(consumer, pose, -size, -size, -size, size, length, nearAlpha);
        beamFace(consumer, pose, size, -size, size, size, length, nearAlpha);
        poseStack.popPose();
    ...
    private static void beamFace(VertexConsumer consumer, PoseStack.Pose pose, double x0, double z0, double x1, double z1,
            double length, int nearAlpha) {
