...
public class ResearchReactorRenderer implements BlockEntityRenderer<ResearchReactorBlockEntity> {
    ...
    @Override
    public boolean shouldRender(ResearchReactorBlockEntity blockEntity, Vec3 cameraPos) {
    ...
    public void render(ResearchReactorBlockEntity blockEntity, float partialTick, PoseStack poseStack,
        ...
        if (totalFlux > 10 && blockEntity.isSubmerged()) {
            LegacyMachineEffectPresenter.enqueueUntexturedQuadGroup(PresentStage.AFTER_BLOCK_ENTITIES, poseStack,
                    ...
                    group -> renderCherenkov(totalFlux, group));
        }
        poseStack.popPose();
    }

    private static void renderCherenkov(int totalFlux, UntexturedQuadGroup group) {
        ...
        for (double d = LegacyTileRenderPlans.SMALL_REACTOR_CHERENKOV_START;
            ...
            renderCherenkovBox(group, d, bottom, top, alphaByte);
        }
    }

    private static void renderCherenkovBox(UntexturedQuadGroup group,
            double d, double bottom, double top, int alpha) {
        quad(group, d, bottom, -d, d, top, -d, d, top, d, d, bottom, d, alpha);
        quad(group, -d, bottom, -d, -d, top, -d, -d, top, d, -d, bottom, d, alpha);
        quad(group, -d, bottom, d, -d, top, d, d, top, d, d, bottom, d, alpha);
        quad(group, -d, bottom, -d, -d, top, -d, d, top, -d, d, bottom, -d, alpha);
        quad(group, -d, top, -d, -d, top, d, d, top, d, d, top, -d, alpha);
        quad(group, -d, bottom, -d, -d, bottom, d, d, bottom, d, d, bottom, -d, alpha);
    }

    private static void quad(UntexturedQuadGroup group,
