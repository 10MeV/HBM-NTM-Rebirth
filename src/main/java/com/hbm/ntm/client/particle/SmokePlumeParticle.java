...
public class SmokePlumeParticle extends TextureSheetParticle {
    ...
    private void renderQuad(VertexConsumer consumer, Quaternionf rotation, Vector3f[] corners,
        ...
        consumer.vertex(corner0.x(), corner0.y(), corner0.z()).uv(u1, v1).color(color, color, color, alpha).uv2(LightTexture.FULL_BRIGHT).endVertex();
        consumer.vertex(corner1.x(), corner1.y(), corner1.z()).uv(u1, v0).color(color, color, color, alpha).uv2(LightTexture.FULL_BRIGHT).endVertex();
        consumer.vertex(corner2.x(), corner2.y(), corner2.z()).uv(u0, v0).color(color, color, color, alpha).uv2(LightTexture.FULL_BRIGHT).endVertex();
        consumer.vertex(corner3.x(), corner3.y(), corner3.z()).uv(u0, v1).color(color, color, color, alpha).uv2(LightTexture.FULL_BRIGHT).endVertex();
    }
