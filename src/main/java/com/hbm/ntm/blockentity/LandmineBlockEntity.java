...
public class LandmineBlockEntity extends BlockEntity {
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, LandmineBlockEntity mine) {
        if (level.isClientSide() || !(state.getBlock() instanceof LandmineBlock block)) {
            return;
        }
        double range = mine.waitingForPlayer ? 25.0D : block.triggerRange() * (mine.primed ? 1.0D : 2.0D);
        double height = mine.waitingForPlayer ? 25.0D : block.triggerHeight() * (mine.primed ? 1.0D : 2.0D);
